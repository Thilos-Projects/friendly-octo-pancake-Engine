...
#define getcwd _getcwd
	...
	void shutdown()
	{
		VLOG_F(g_internal_verbosity, "FrameWork_Logger_loguru::shutdown()");
	...
	*/
	...
	static void log_message(int stack_trace_skip, Message & message, bool with_indentation, bool abort_if_fatal)
	{
		...
		if (message.verbosity == Verbosity_FATAL) {
			auto st = FrameWork_Logger_loguru::stacktrace(stack_trace_skip + 2);
			if (!st.empty()) {
				RAW_LOG_F(ERROR, "Stack trace:\n" LOGURU_FMT(s) "", st.c_str());
			}

			auto ec = FrameWork_Logger_loguru::get_error_context();
			if (!ec.empty()) {
	...
	{
		auto message = _ss.str();
		FrameWork_Logger_loguru::log_and_abort(1, _expr, _file, _line, LOGURU_FMT(s), message.c_str());
	}
