...
# Version history
	...
	* Version 1.2.3 - 2016-05-16 - Log current working directory in FrameWork_Logger_loguru::init().
	* Version 1.2.4 - 2016-05-18 - Custom replacement for -v in FrameWork_Logger_loguru::init() by bjoernpollex
	...
	* Version 1.3.2 - 2016-07-20 - Add FrameWork_Logger_loguru::arguments()
	* Version 1.4.0 - 2016-09-15 - Semantic versioning + add FrameWork_Logger_loguru::create_directories
	...
	* Version 1.7.0 - 2018-01-03 - Add ability to turn off the preamble with FrameWork_Logger_loguru::g_preamble
	...
	* Version 2.1.0 - 2019-09-23 - Update fmtlib + add option to FrameWork_Logger_loguru::init to NOT set main thread name.
...
#error "You are defining LOGURU_CATCH_SIGABRT. This is for older versions of Loguru. You should now instead set the options passed to FrameWork_Logger_loguru::init"
#endif
...
#error "You are defining LOGURU_UNSAFE_SIGNAL_HANDLER. This is for older versions of Loguru. You should now instead set the unsafe_signal_handler option when you call FrameWork_Logger_loguru::init."
#endif
...
namespace FrameWork_Logger_loguru
{
...
#undef MAX
	...
	Set to FrameWork_Logger_loguru::Verbosity_OFF to write nothing to stderr.
	...
	*/
	...
	// Runtime options passed to FrameWork_Logger_loguru::init
	struct Options
	{
		...
		// FrameWork_Logger_loguru::init will set the name of the calling thread to this.
		// If you don't want Loguru to set the name of the main thread,
		// set this to nullptr.
		// NOTE: on SOME platforms FrameWork_Logger_loguru::init will only overwrite the thread name
		// if a thread name has not already been set.
		// To always set a thread name, use FrameWork_Logger_loguru::set_thread_name instead.
	...
	/*  Should be called from the main thread.
		...
		FrameWork_Logger_loguru::init() will look for arguments meant for loguru and remove them.
		Arguments meant for loguru are:
			-v n   Set FrameWork_Logger_loguru::g_stderr_verbosity level. Examples:
		...
		Tip: You can set g_stderr_verbosity before calling FrameWork_Logger_loguru::init.
		That way you can set the default but have the user override it with the -v flag.
		Note that -v does not affect file logging (see FrameWork_Logger_loguru::add_file).
	...
	*/
	...
	// Returns all arguments given to FrameWork_Logger_loguru::init(), but escaped with a single space as separator.
	LOGURU_EXPORT
		const char* arguments();

	// Returns the path to the current working dir when FrameWork_Logger_loguru::init() was called.
	LOGURU_EXPORT
	...
	*/
	...
	/*  Will log to a file at the given path.
		...
		If path starts with a ~, it will be replaced with FrameWork_Logger_loguru::home_dir()
		To stop the file logging, just call FrameWork_Logger_loguru::remove_callback(path) with the same path.
	*/
	LOGURU_EXPORT
	...
	*/
	...
	/* Generates a readable stacktrace as a string.
	   ...
	   don't include the call to FrameWork_Logger_loguru::stacktrace in the stack trace. */
	LOGURU_EXPORT
	...
	/*  Add a string to be replaced with something else in the stack output.
		...
		You can clean it up with:
			auto verbose_type_name = FrameWork_Logger_loguru::demangle(typeid(std::ofstream).name());
			FrameWork_Logger_loguru::add_stack_cleanup(verbose_type_name.c_str(); "std::ofstream");
	...
	*/
	...
	Usage:
		printf("%sRed%sGreen%sBold green%sClear again\n",
			   FrameWork_Logger_loguru::terminal_red(), FrameWork_Logger_loguru::terminal_green(),
			   FrameWork_Logger_loguru::terminal_bold(), FrameWork_Logger_loguru::terminal_reset());
	...
	*/
	...
	/* 	A stack trace gives you the names of the function at the point of a crash.
		...
		Use FrameWork_Logger_loguru::get_error_context() to get the contents of the active error contexts.
		Example result:
		------------------------------------------------
...
#define ERROR_CONTEXT(descr, data)                                             \
		const FrameWork_Logger_loguru::EcEntryData<FrameWork_Logger_loguru::make_ec_type<decltype(data)>::type>      \
			LOGURU_ANONYMOUS_VARIABLE(error_context_scope_)(                       \
				__FILE__, __LINE__, descr, data,                                   \
				static_cast<FrameWork_Logger_loguru::EcEntryData<FrameWork_Logger_loguru::make_ec_type<decltype(data)>::type>::Printer>(FrameWork_Logger_loguru::ec_to_text) ) // For better error messages

	/*
		#define ERROR_CONTEXT(descr, data)                                 \
			const auto LOGURU_ANONYMOUS_VARIABLE(error_context_scope_)(    \
				FrameWork_Logger_loguru::make_ec_entry_lambda(__FILE__, __LINE__, descr,    \
					[=](){ return FrameWork_Logger_loguru::ec_to_text(data); }))
	*/
	...
	/*
		...
		You can pass the handle to FrameWork_Logger_loguru::get_error_context on another thread.
		...
		void foo(const char* parameter)
		{
			...
			const auto parent_ec_handle = FrameWork_Logger_loguru::get_thread_ec_handle();
			std::thread([=]{
				FrameWork_Logger_loguru::set_thread_name("child thread");
...
#define VLOG_F(verbosity, ...)                                                                     \
	((verbosity) > FrameWork_Logger_loguru::current_verbosity_cutoff()) ? (void)0                                   \
									  : FrameWork_Logger_loguru::log(verbosity, __FILE__, __LINE__, __VA_ARGS__)

// LOG_F(INFO, "Foo: %d", some_number);
#define LOG_F(verbosity_name, ...) VLOG_F(FrameWork_Logger_loguru::Verbosity_ ## verbosity_name, __VA_ARGS__)

#define VLOG_IF_F(verbosity, cond, ...)                                                            \
	((verbosity) > FrameWork_Logger_loguru::current_verbosity_cutoff() || (cond) == false)                          \
		? (void)0                                                                                  \
		: FrameWork_Logger_loguru::log(verbosity, __FILE__, __LINE__, __VA_ARGS__)

#define LOG_IF_F(verbosity_name, cond, ...)                                                        \
	VLOG_IF_F(FrameWork_Logger_loguru::Verbosity_ ## verbosity_name, cond, __VA_ARGS__)

#define VLOG_SCOPE_F(verbosity, ...)                                                               \
	FrameWork_Logger_loguru::LogScopeRAII LOGURU_ANONYMOUS_VARIABLE(error_context_RAII_) =                          \
	((verbosity) > FrameWork_Logger_loguru::current_verbosity_cutoff()) ? FrameWork_Logger_loguru::LogScopeRAII() :                  \
	FrameWork_Logger_loguru::LogScopeRAII(verbosity, __FILE__, __LINE__, __VA_ARGS__)

// Raw logging - no preamble, no indentation. Slightly faster than full logging.
#define RAW_VLOG_F(verbosity, ...)                                                                 \
	((verbosity) > FrameWork_Logger_loguru::current_verbosity_cutoff()) ? (void)0                                   \
									  : FrameWork_Logger_loguru::raw_log(verbosity, __FILE__, __LINE__, __VA_ARGS__)

#define RAW_LOG_F(verbosity_name, ...) RAW_VLOG_F(FrameWork_Logger_loguru::Verbosity_ ## verbosity_name, __VA_ARGS__)

// Use to book-end a scope. Affects logging on all threads.
#define LOG_SCOPE_F(verbosity_name, ...)                                                           \
	VLOG_SCOPE_F(FrameWork_Logger_loguru::Verbosity_ ## verbosity_name, __VA_ARGS__)
...
// Message is optional
#define ABORT_F(...) FrameWork_Logger_loguru::log_and_abort(0, "ABORT: ", __FILE__, __LINE__, __VA_ARGS__)

// --------------------------------------------------------------------
// CHECK_F macros:

#define CHECK_WITH_INFO_F(test, info, ...)                                                         \
	LOGURU_PREDICT_TRUE((test) == true) ? (void)0 : FrameWork_Logger_loguru::log_and_abort(0, "CHECK FAILED:  " info "  ", __FILE__,      \
...
#define CHECK_OP_F(expr_left, expr_right, op, ...)                                                 \
	...
	{                                                                                              \
		...
		{                                                                                          \
			auto str_left = FrameWork_Logger_loguru::format_value(val_left);                                        \
			auto str_right = FrameWork_Logger_loguru::format_value(val_right);                                      \
			auto fail_info = FrameWork_Logger_loguru::textprintf("CHECK FAILED:  " LOGURU_FMT(s) " " LOGURU_FMT(s) " " LOGURU_FMT(s) "  (" LOGURU_FMT(s) " " LOGURU_FMT(s) " " LOGURU_FMT(s) ")  ",           \
				#expr_left, #op, #expr_right, str_left.c_str(), #op, str_right.c_str());           \
			auto user_msg = FrameWork_Logger_loguru::textprintf(__VA_ARGS__);                                       \
			FrameWork_Logger_loguru::log_and_abort(0, fail_info.c_str(), __FILE__, __LINE__,                        \
...
#define VLOG_IF_S(verbosity, cond)                                                                 \
	((verbosity) > FrameWork_Logger_loguru::current_verbosity_cutoff() || (cond) == false)                          \
		? (void)0                                                                                  \
		: FrameWork_Logger_loguru::Voidify() & FrameWork_Logger_loguru::StreamLogger(verbosity, __FILE__, __LINE__)
#define LOG_IF_S(verbosity_name, cond) VLOG_IF_S(FrameWork_Logger_loguru::Verbosity_ ## verbosity_name, cond)
#define VLOG_S(verbosity)              VLOG_IF_S(verbosity, true)
#define LOG_S(verbosity_name)          VLOG_S(FrameWork_Logger_loguru::Verbosity_ ## verbosity_name)

// -----------------------------------------------
// ABORT_S macro. Usage:  ABORT_S() << "Causo of error: " << details;

#define ABORT_S() FrameWork_Logger_loguru::Voidify() & FrameWork_Logger_loguru::AbortLogger("ABORT: ", __FILE__, __LINE__)
...
#define CHECK_WITH_INFO_S(cond, info)                                                              \
	LOGURU_PREDICT_TRUE((cond) == true)                                                            \
		? (void)0                                                                                  \
		: FrameWork_Logger_loguru::Voidify() & FrameWork_Logger_loguru::AbortLogger("CHECK FAILED:  " info "  ", __FILE__, __LINE__)
...
#define CHECK_OP_S(function_name, expr1, op, expr2)                                                \
	while (auto error_string = FrameWork_Logger_loguru::function_name(#expr1 " " #op " " #expr2,                    \
													 FrameWork_Logger_loguru::referenceable_value(expr1), #op,      \
													 FrameWork_Logger_loguru::referenceable_value(expr2)))          \
		FrameWork_Logger_loguru::AbortLogger(error_string->c_str(), __FILE__, __LINE__)
...
#define DVLOG_IF_S(verbosity, cond)                                                     \
		(true || (verbosity) > FrameWork_Logger_loguru::current_verbosity_cutoff() || (cond) == false)       \
			? (void)0                                                                       \
			: FrameWork_Logger_loguru::Voidify() & FrameWork_Logger_loguru::StreamLogger(verbosity, __FILE__, __LINE__)

#define DLOG_IF_S(verbosity_name, cond) DVLOG_IF_S(FrameWork_Logger_loguru::Verbosity_ ## verbosity_name, cond)
#define DVLOG_S(verbosity)              DVLOG_IF_S(verbosity, true)
#define DLOG_S(verbosity_name)          DVLOG_S(FrameWork_Logger_loguru::Verbosity_ ## verbosity_name)
#endif
...
#define VLOG_IS_ON(verbosity) ((verbosity) <= FrameWork_Logger_loguru::current_verbosity_cutoff())
